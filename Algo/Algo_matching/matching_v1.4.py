...
for c_name in companies.keys():
    ...
    for dp in disliked_profiles:
        dislike_sims.append(compute_cosine_sim(company_profiles[c_name], dp))
...
for c_name in companies.keys():
    ...
    if c_name in liked_companies:
        rating = 5.0
    elif c_name in disliked_companies:
        rating = 1.0
    else:
        # 0~max_sim -> 1~5 스케일링
...
for c_name in companies.keys():
    ...
    # pred.est 범위는 보통 1~5 근처
    cf_scores[c_name] = pred.est

######################################
# 7. 점수 정규화(선택) & 하이브리드 결합
######################################
# - content_scores, cf_scores는 각각 0~? or 1~5 등 범위가 다를 수 있음
# - min-max 정규화로 0~1 범위로 맞춘 뒤, ALPHA/ BETA 가중합
...
for c_name in companies.keys():
    ...
    hybrid_score = ALPHA*c_score + BETA*cf_score  # 하이브리드 가중합
