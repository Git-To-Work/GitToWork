...
#############################################
# 1. 기본 설정 및 사용자 데이터 로딩
#############################################
username = "chanhoan"

# 추가 신호 집합 (좋아요, 블랙리스트, 구독, 조회)
liked_companies_set = {"현대자동차"}
blacklisted_companies_set = {"에프엘이에스"}
scraped_companies_set = {"클로비스"}  # 구독한 기업
user_search_detail_history = {"애자일소다"}  # 조회한 기업

# 하이브리드 결합 가중치 (콘텐츠 vs. CF)
ALPHA = 0.9  # 콘텐츠 기반 점수 비중
BETA = 0.1   # 협업 필터링 점수 비중

# 콘텐츠 기반 내 추가 신호 가중치
LIKED_BONUS = 0.3        # 좋아요: 중간 긍정
BLACKLISTED_PENALTY = 0.5  # 블랙리스트: 큰 부정
...
# 복잡도 메트릭 (각 언어별 average_cyclomatic_complexity)
complexity_metrics = user_data.get("complexity_metrics", {})

# 사용자가 사용한 언어 중, 복잡도 정보가 있고 값이 0보다 큰 항목만 사용
complexity_values = []
...
# 각 언어별 "효과적인" 커밋 수 = commit_count * (min_complexity / 해당 언어의 복잡도)
...
# README 분석: Flesch Reading Ease 기준 토큰
...
# 최종 사용자 프로필 텍스트: **가중치 적용된 텍스트와 읽기 토큰만 사용**
...
#############################################
# 공고 데이터
job_files = glob(os.path.join(".", "jobs", "*.json"))
jobs = []
...
# 기업 데이터
company_files = glob(os.path.join(".", "crawling", "*.json"))
companies = {}
...
content_scores = {}
# 미리 좋아요/블랙리스트 기업 프로필 추출
liked_profiles = [company_profiles[c] for c in liked_companies_set if c in company_profiles]
blacklisted_profiles = [company_profiles[c] for c in blacklisted_companies_set if c in company_profiles]

for comp_name in companies.keys():
    # 기본: 사용자 vs. 기업 콘텐츠 유사도
    ...
    # 블랙리스트 신호
    blacklisted_sims = [compute_cosine_sim(company_profiles[comp_name], bp) for bp in blacklisted_profiles]
    avg_blacklisted = avg(blacklisted_sims)
    
    final_content_score = user_sim + LIKED_BONUS * avg_like - BLACKLISTED_PENALTY * avg_blacklisted
    
    # 추가 신호: scraped 및 user_search_detail
    if comp_name in scraped_companies_set:
        final_content_score += SCRAPED_BONUS
    if comp_name in user_search_detail_history:
...
for comp in companies.keys():
    ...
    if comp in liked_companies_set:
        rating = 5.0
    elif comp in blacklisted_companies_set:
        rating = 1.0
    else:
