...
# 현재 날짜-시간을 "YYYYMMDD-HHMMSS" 형식으로 생성하여 파일 이름에 사용
current_time_str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
file_name = f"{current_time_str}_combined_report.json"

# 저장 경로 예시: 현재 작업 디렉토리의 user 폴더 내 (레포지토리 소유자는 첫번째 레포지토리의 소유자 이름으로 사용)
first_repo_owner = all_reports[0]["repository"].split("/")[0] if all_reports else "unknown"
output_dir = os.path.join(".", "user", first_repo_owner)
...
print(f"모든 레포지토리 분석 보고서가 저장되었습니다: {output_path}")
